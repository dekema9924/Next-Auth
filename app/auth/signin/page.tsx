...
import { SendOtp, Signin, SignInWithSocial, Signup } from '@/lib/actions/auth-actions';
    ...
    const onSubmit: SubmitHandler<FormData> = async (data) => {
        ...
        if (activeTab === 'signup') {
            ...
            try {
                ...
                if (res.user.user && !res.user.user.emailVerified) {
            ...
            try {
                ...
                if (res.user && !res.user.response.user.emailVerified) {
                    const otp = await SendOtp(data?.email);
                    console.log(otp)
                    toast.success("OTP sent to your email. Please verify your email to proceed.");
                    router.push('/auth/verify-email');
                    return;
                } else {
        ...
        <div className="min-h-screen bg-linear-to-b from-[#050816] via-[#0f1535] to-[#1e3a8a] flex items-center justify-center p-4">
            ...
            <div className="bg-[#151a30] rounded-3xl shadow-2xl w-full max-w-md p-8 relative border border-[#1f2544]">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button onClick={() => SignInWithSocial('google')} type="button" className="cursor-pointer flex items-center justify-center gap-2 bg-[#1a1f38] hover:bg-[#1f2540] text-white rounded-lg py-3 transition-all border border-[#252b47]">
                        ...
                        <button onClick={() => SignInWithSocial('github')} type="button" className="flex cursor-pointer items-center justify-center gap-2 bg-[#1a1f38] hover:bg-[#1f2540] text-white rounded-lg py-3 transition-all border border-[#252b47]">
